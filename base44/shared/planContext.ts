...
export function computeBaseSlots(profile) {
  const desiredByDay = {};
  (profile?.desired_activities || []).forEach((a) => {
...
export function computeFeedbackDigest(feedbackList) {
...
export function buildProfileContext(profile, feedback) {
  ...
  const duration = profile?.duration_mode === 'per_day'
    ? 'per day: ' + (profile.training_days || []).map((d) => `${d}:${profile.per_day_durations?.[d] ?? 45}min`).join(', ')
    : `${profile?.duration_min ?? 45}–${profile?.duration_max ?? 60} min per session`;
  const scheduled = (profile?.scheduled_activities || []).map((a) => `${a.day} ${a.time_of_day}: ${a.activity}`).join('; ') || 'none';
  const desired = (profile?.desired_activities || []).map((a) => `${a.day}: ${a.activity}`).join('; ') || 'none';
  const calibration = profile?.calibrated && profile?.strength_calibration?.length
    ? profile.strength_calibration.map((c) => `${c.pattern}: ${c.exercise} @ ${c.weight_kg}kg x ${c.reps || '?'}`).join('; ')
...
export function buildWorkoutCatalog(workouts) {
