...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-background shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
        top: "inset-x-0 top-0 border-b max-h-[100dvh] overflow-y-auto data-[state=closed]:slide-out-to-top data-[state=open]:slide-in-from-top",
        bottom:
          "inset-x-0 bottom-0 border-t max-h-[100dvh] overflow-y-auto data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom",
...
/**
 * @typedef {React.ComponentPropsWithoutRef<typeof SheetPrimitive.Content> &
 *   import("class-variance-authority").VariantProps<typeof sheetVariants>} SheetContentProps
 */

/** @type {React.ForwardRefExoticComponent<SheetContentProps & React.RefAttributes<React.ElementRef<typeof SheetPrimitive.Content>>>} */
...
/** @type {React.FC<React.ComponentPropsWithoutRef<"div">>} */
const SheetHeader = ({
  className,
  ...props
}) => (
